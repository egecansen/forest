...
#   <proj>/.cursor/skills/hektor-flaky-triage/{core,hooks}
#   <proj>/.cursor/hooks{,/lib}          (both gates + vendored libs)
...
PROJ="$WORK/proj"
SKILL="$PROJ/.cursor/skills/hektor-flaky-triage"
mkdir -p "$SKILL/core" "$PROJ/.cursor/hooks/lib" "$PROJ/.cursor/hooks/lib" "$PROJ/.cursor/rules"
git -C "$PROJ" init -q
...
cp "$KITSRC/gates/flaky-kit-self-protection-gate.sh" "$PROJ/.cursor/hooks/flaky-kit-self-protection-gate.sh"
...
cp "$KITSRC/gates/flaky-kit-delivery-gate.sh"        "$PROJ/.cursor/hooks/flaky-kit-delivery-gate.sh"
cp "$KITSRC/gates/lib/audit.sh"                             "$PROJ/.cursor/hooks/lib/audit.sh"
cp "$KITSRC/gates/flaky-kit-self-protection-gate.sh"  "$PROJ/.cursor/hooks/flaky-kit-self-protection-gate.sh"
cp "$KITSRC/gates/lib/cursor.sh"               "$PROJ/.cursor/hooks/lib/cursor.sh"
cp "$KITSRC/gates/lib/audit.sh"                             "$PROJ/.cursor/hooks/lib/audit.sh"
chmod +x "$SKILL/core/shell-guard.py" "$SKILL/core/lock-kit.sh" \
         "$PROJ/.cursor/hooks/flaky-kit-self-protection-gate.sh" "$PROJ/.cursor/hooks/flaky-kit-delivery-gate.sh" \
...
UNLOCK_OUT=$(cd "$PROJ" && printf '%s' "$(cursor_bash_json "$SKILL" "(cd core && sed -i '' config.json)")" \
  | HEKTOR_FLAKYKIT_UNLOCK=1 "$CURSOR_GATE" 2>/dev/null)
[ -z "$UNLOCK_OUT" ] && ok || bad "HEKTOR_FLAKYKIT_UNLOCK=1 still DENIES the subshell-bypass case, got: $UNLOCK_OUT"
...
echo "== Fix 2 (Task 5): relocated Claude gate lives at .cursor/hooks/, out of the shadowable kit tree ==" >&2

assert_cursor_edit_deny  "$PROJ/.cursor/hooks/flaky-kit-self-protection-gate.sh" "the gate script itself"
assert_cursor_edit_deny  "$PROJ/.cursor/hooks/lib/audit.sh"                      "the audit lib"
assert_cursor_edit_deny  "$PROJ/.cursor/hooks/flaky-kit-self-protection-gate.sh" "cursor gate self-protect"
assert_cursor_edit_deny  "$PROJ/.cursor/hooks/lib/cursor.sh"              "cursor lib self-protect"
assert_cursor_edit_allow "$PROJ/.cursor/hooks.json"                          "an unrelated hooks.json edit"
...
GATE_MUT_CMD="sed -i '' .cursor/hooks/flaky-kit-self-protection-gate.sh"
cursor_denied "$(cursor_bash_json "$PROJ" "$GATE_MUT_CMD")" \
  && ok || bad "DENY Bash mutation of the relocated gate itself (shell-guard.py SURF): $GATE_MUT_CMD"

echo "== Directory OPERANDS: every surface pattern used to end in '/', so the directory itself never" >&2
echo "   matched — 'mv <kit> /tmp/x', 'rm -rf <kit>', 'mv <kit>/core /tmp/x' and 'rm -rf .cursor/hooks'" >&2
...
proj_bash_deny "rm -rf $PROJ/.cursor/hooks"           # takes the gate, its lib AND the record with it
...
# ALLOW+silent.
proj_bash_deny "printf 'degraded\\n' > $PROJ/.cursor/hooks/.flaky-kit-expect"
proj_bash_deny "rm -f $PROJ/.cursor/hooks/.flaky-kit-expect"
assert_cursor_edit_deny "$PROJ/.cursor/hooks/.flaky-kit-expect" "the out-of-tree lock-tier record (parity)"
...
echo "   .cursor/hooks/flaky-kit-delivery-gate.sh' was an ALLOW with no audit line while the same" >&2
...
DG="$PROJ/.cursor/hooks/flaky-kit-delivery-gate.sh"
...
SURF_RE_DG="$(sed -n "s/^SURF_RE='\(.*\)'\$/\1/p" "$CURSOR_GATE")"
printf '%s' "rm -f $DG" | grep -qE "$SURF_RE_DG" \
  && ok || bad "the gates' bash SURF_RE fallback must also class the delivery gate as surface"
# The delivery gate lands in this same directory, so erasing it there is a surface write like any
# other. This used to be an ALLOW control on the reasoning that Cursor had no stop event and the file
# could never exist — that reasoning is obsolete: `stop` exists, install.sh puts the gate here, and
# `rm -f` on it is exactly the cheapest way to unwire the I11 check.
proj_bash_deny "rm -f $PROJ/.cursor/hooks/flaky-kit-delivery-gate.sh"
...
# An ordinary file INSIDE .cursor/hooks/ stays editable: those are this pack's other hooks, they are
...
proj_bash_allow "rm -f $PROJ/.cursor/hooks/observe.sh"
...
for s in ".cursor/hooks.json" ".cursor/hooks.json" ".cursor/hooks.json"; do
...
proj_bash_deny "printf '{}' > .cursor/hooks.json"
...
# --- Write/Edit on hooks.json: deny only when the registration would not survive -------
# Cursor's shape is FLAT: .hooks.<event>[] entries carrying `command` and an optional
# `matcher`. There is one registration file, so this is the only shape to test.
SJ="$PROJ/.cursor/hooks.json"
GCMD=".cursor/hooks/flaky-kit-self-protection-gate.sh"
DGC=".cursor/hooks/flaky-kit-delivery-gate.sh"
REG="$(jq -nc --arg g "$GCMD" '{version:1, hooks:{
  preToolUse:[{command:$g, matcher:"Write|Edit", timeout:10}],
  beforeShellExecution:[{command:$g, timeout:10}]}}')"
printf '%s\n' "$REG" > "$SJ"

# A Write that drops the registration -> DENY
assert_cursor_write_deny "$SJ" '{"version":1,"hooks":{"preToolUse":[]}}' \
...
assert_cursor_write_allow "$SJ" "$(printf '%s' "$REG" | jq -c '.version = 2')" \
  "a Write that keeps the registration must be allowed even though it touches hooks.json"
# Unparseable proposed JSON -> DENY (survival cannot be verified, and writing broken config is
...
# An Edit that carries the shell registration away -> DENY. Note WHICH registration: only the
# beforeShellExecution arm. `_reg_slots` (not "does the gate's filename still appear somewhere") is
# what makes this a DENY — the preToolUse arm survives untouched, so a "one mention is enough" test
# would call it a survival and allow the shell branch to be unwired. core/_integrity.sh's wiring axis
# rules the same way, on the same slot model.
assert_cursor_edit_str_deny "$SJ" \
  '"beforeShellExecution":[{"command":".cursor/hooks/flaky-kit-self-protection-gate.sh","timeout":10}]' \
  '"beforeShellExecution":[]' \
  "an Edit that removes the beforeShellExecution registration must be denied"
# An Edit that touches an unrelated key -> ALLOW
assert_cursor_edit_str_allow "$SJ" '"version":1' '"version":1' \
...
echo "   and its outcome test used to read the preToolUse arm and nothing else — so a Write applying" >&2
echo "   del(.hooks.stop) with that arm left intact was a verified ALLOW: every key the check read was" >&2
echo "   untouched, and it was blind to the one key the write removed. Unregistering the stop hook is" >&2
echo "   the cheapest way to end a session the stop hook has just refused to let end. ==" >&2
REGS="$(printf '%s' "$REG" | jq -c --arg d "$DGC" '.hooks.stop = [{command:$d, timeout:20, loop_limit:1}]')"
printf '%s\n' "$REGS" > "$SJ"
...
assert_cursor_edit_str_deny "$SJ" \
  '"stop":[{"command":".cursor/hooks/flaky-kit-delivery-gate.sh","timeout":20,"loop_limit":1}]' \
  '"stop":[]' \
...
assert_cursor_write_allow "$SJ" "$(printf '%s' "$REGS" | jq -c '.version = 3')" \
  "a Write that keeps the stop registration must be allowed even though it touches hooks.json"
...
assert_cursor_write_deny "$SJ" "$(printf '%s' "$REGS" | jq -c --arg g "$GCMD" '.hooks.stop[0].command = $g')" \
...
# covering the self-protection gate's tools.
DGP="$(jq -nc --arg g "$GCMD" --arg d "$DGC" '{version:1, hooks:{preToolUse:[
  {command:$g, matcher:"Write|Edit"},
  {command:$g, matcher:"Bash"},
  {command:$d, matcher:"Bash"}]}}')"
printf '%s\n' "$DGP" > "$SJ"
...
# and every pre-delivery-gate project would be frozen out of its own hooks.json.
assert_cursor_write_allow "$SJ" "$(printf '%s' "$REG" | jq -c '.version = 4')" \
  "a hooks.json that never had a stop registration must not start needing one"
...
# An absent matcher covers everything, so dropping it loses no coverage.
assert_cursor_write_allow "$SJ" "$(printf '%s' "$REG" | jq -c 'del(.hooks.preToolUse[0].matcher)')" \
  "an absent matcher covers every tool the matcher covered, so it must be allowed"
...
DECOY="$(jq -nc --arg g "$GCMD" '{
  note:"flaky-kit-self-protection-gate.sh",
  version:1,
  hooks:{preToolUse:[{command:$g, matcher:"Write|Edit"}],
         beforeShellExecution:[{command:$g}]}}')"
printf '%s\n' "$DECOY" > "$SJ"
...
assert_cursor_edit_all_allow "$SJ" '"timeout":10' '"timeout":15' true \
...
assert_cursor_write_allow "$SJ" '{"version":1,"hooks":{"preToolUse":[]}}' \
...
# that never installed this kit out of its own config. Without this control the early
...
echo "== The gate defines the pieces the cross-axis checks below read ==" >&2
# There used to be two gate scripts, one per harness, and this block asserted their SURF_RE and slot
# readers stayed byte-identical. There is one gate now, so that comparison is vacuous — but the
# pieces it guarded are still read by the cross-axis parity check below, and a rename or refactor
# that removes one would make that check silently compare nothing. So assert they EXIST.
[ -n "$(grep -m1 '^SURF_RE=' "$CURSOR_GATE")" ] && ok || bad "the gate must define SURF_RE (the cross-axis drift check reads it)"
for fn in _reg_slots _slots_kept; do
  grep -q "^${fn}() {" "$CURSOR_GATE" && ok || bad "the gate must define $fn() (the cross-axis slot parity check reads it)"
done
...
JQ="$(command -v jq)"
eval "$(sed -n '/^_reg_slots() {/,/^}/p' "$CURSOR_GATE")"
. "$CORE/_integrity.sh"
PARITY_DOC_DIR="$WORK/slot-parity"; mkdir -p "$PARITY_DOC_DIR"
SLOT_DRIFT=0; n=0
gate_c=".cursor/hooks/flaky-kit-self-protection-gate.sh"
dlv_c=".cursor/hooks/flaky-kit-delivery-gate.sh"
# Cursor's flat shape throughout: .hooks.<event>[] entries with `command` and an optional `matcher`.
for doc in \
  "$(jq -nc --arg c "$gate_c" --arg d "$dlv_c" '{version:1,hooks:{preToolUse:[{command:$c,matcher:"Write|Edit"},{command:$c,matcher:"Bash"}],stop:[{command:$d}]}}')" \
  "$(jq -nc --arg d "$dlv_c" '{version:1,hooks:{stop:[{command:$d}]}}')" \
  "$(jq -nc --arg c "$gate_c" '{version:1,hooks:{stop:[{command:$c}]}}')" \
  "$(jq -nc --arg d "$dlv_c" '{version:1,hooks:{preToolUse:[{command:$d,matcher:"Bash"}]}}')" \
  '{"version":1,"hooks":{"stop":[]}}' \
  "$(jq -nc --arg c "$gate_c" '{version:1,hooks:{preToolUse:[{command:$c,matcher:"Write|Edit"},{command:$c,matcher:"Bash"}]}}')" \
  "$(jq -nc --arg c "$gate_c" '{version:1,hooks:{preToolUse:[{command:$c,matcher:"Write|Edit|MultiEdit"},{command:$c,matcher:"Bash"}]}}')" \
  "$(jq -nc --arg c "$gate_c" '{version:1,hooks:{preToolUse:[{command:$c,matcher:"*"}]}}')" \
  "$(jq -nc --arg c "$gate_c" '{version:1,hooks:{preToolUse:[{command:$c}]}}')" \
  "$(jq -nc --arg c "$gate_c" '{version:1,hooks:{preToolUse:[{command:$c,matcher:"Write"}]}}')" \
  "$(jq -nc --arg c "$gate_c" '{version:1,hooks:{beforeShellExecution:[{command:$c}],preToolUse:[{command:$c,matcher:"Write|Edit"}]}}')" \
  "$(jq -nc --arg c "$gate_c" '{version:1,hooks:{preToolUse:[{command:$c,matcher:"Task"}]}}')" \
  "$(jq -nc --arg c "$gate_c" '{version:1,hooks:{preToolUse:[{command:$c,matcher:"*"}]}}')" \
  '{"version":1,"hooks":{"preToolUse":[]}}' \
  '{"version":1,"hooks":{"preToolUse":[{"matcher":"Write|Edit","command":"/some/other/hook.sh"}]}}'
do
...
SURF_RE_VAL="$(sed -n "s/^SURF_RE='\(.*\)'\$/\1/p" "$CURSOR_GATE")"
...
for p in "$SKILL/core/config.json" "$SKILL/SKILL.md" "$SKILL" "$SKILL/core" \
         "$PROJ/.cursor/hooks/flaky-kit-self-protection-gate.sh" "$PROJ/.cursor/hooks/.flaky-kit-expect" \
         "$PROJ/.cursor/hooks/flaky-kit-delivery-gate.sh" "$PROJ/.cursor/hooks/flaky-kit-delivery-gate.sh" \
         "$PROJ/.cursor/hooks/lib/audit.sh" "$PROJ/.cursor/hooks/lib/audit.sh" \
         "$PROJ/.cursor/hooks/flaky-kit-self-protection-gate.sh" \
         "$PROJ/.cursor/hooks" "$PROJ/.cursor/hooks" \
         "$PROJ/.cursor/hooks/observe.sh" "$PROJ/.cursor/hooks.json" "$PROJ/README.md" "/tmp/elsewhere" \
         "$PROJ/.cursor/hooks.json" "$PROJ/.cursor/hooks.json"
do
...
[ "$?" -eq 0 ] && ok || bad "HEKTOR_FK_SURFACE must still ADD a root — a kit installed outside .cursor/skills/ has to be able to protect itself"
...
# record (.cursor/hooks/.flaky-kit-expect) regardless of which harness reads it, so a dual-harness
...
case "$CURSOR_OUT" in *SHADOW*) ok ;; *) bad "the gate must warn when the recorded tier is hardened but the tree at the expected path is NOT root-owned — a rename-and-replace, got: $CURSOR_OUT" ;; esac
...
printf 'unlocked\n' > "$PROJ/.cursor/hooks/.flaky-kit-expect"
CURSOR_OUT="$(printf '{"command":"echo hi","cwd":"%s"}' "$PROJ" | "$CURSOR_GATE" 2>&1)"
case "$CURSOR_OUT" in *SHADOW*) bad "the gate must stay silent while the record says 'unlocked' — a maintenance window is not a shadow" ;; *) ok ;; esac
printf 'hardened\n' > "$PROJ/.cursor/hooks/.flaky-kit-expect"
...
case "$SILENT_CURSOR" in *SHADOW*) bad "the gate must stay SILENT when core/ genuinely reports root-owned (record says hardened, ownership agrees) — got: $SILENT_CURSOR" ;; *) ok ;; esac
...
OUT="$(printf '{"tool_name":"Bash","cwd":"%s","tool_input":{"command":"echo hi"}}' "$PROJ" \
       | "$PROJ/.cursor/hooks/flaky-kit-self-protection-gate.sh" 2>&1)"
