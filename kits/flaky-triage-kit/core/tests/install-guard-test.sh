...
SKILL_DIR="$P/.cursor/skills/hektor-flaky-triage"
OUT="$("$KITSRC/install.sh"  --project "$P" 2>&1)"
...
OUT="$("$KITSRC/install.sh"  --project "$P" 2>&1)"; RC=$?
...
SH
chmod +x "$SHIM/stat"
OUT="$(STAT_TARGET="$SKILL_DIR/core" PATH="$SHIM:$PATH" "$KITSRC/install.sh"  --project "$P" 2>&1)"; RC=$?
...
OLDCMD='"$CLAUDE_PROJECT_DIR/.cursor/skills/hektor-flaky-triage/hooks/flaky-kit-self-protection-gate.sh"'
t="$TMP/settings.tmp"
...
grep -qF 'skills/hektor-flaky-triage/hooks/flaky-kit-self-protection-gate.sh' "$P/.cursor/hooks.json" \
  && ok || bad "fixture precondition: the stale registration must be present before the upgrade run"
"$KITSRC/install.sh"  --project "$P" >/dev/null 2>&1
...
grep -qF '.cursor/hooks/flaky-kit-self-protection-gate.sh' "$P/.cursor/hooks.json" \
  && ok || bad "the stale-registration filter must not remove the relocated gate's own registration"
[ "$(jq '[.hooks.preToolUse[].hooks[].command] | map(select(contains("flaky-kit"))) | length' "$P/.cursor/hooks.json")" = 2 ] \
...
"$KITSRC/install.sh"  --project "$P2" >/dev/null 2>&1
...
cmp -s "$GS/flaky-kit-self-protection-gate.sh" "$P/.cursor/hooks/flaky-kit-self-protection-gate.sh" \
  && ok || bad "the restore source must be byte-identical to the installed gate"

# --- the same, for Cursor -----------------------------------------------------------------------
# install.sh writes gate-src/ symmetrically to gate-src/ above, but nothing had ever
# installed  in this file, so that half rested on by-eye symmetry alone. Task 3
# restores the Cursor gate from this path too.
P2="$TMP/proj-cursor"; mkdir -p "$P2"; git -C "$P2" init -q
SKILL_DIR2="$P2/.cursor/skills/hektor-flaky-triage"
"$KITSRC/install.sh"  --project "$P2" >/dev/null 2>&1
GSC="$SKILL_DIR2/core/gate-src"
...
[ -x "$P/.cursor/hooks/flaky-kit-delivery-gate.sh" ] && ok || bad "install must ship the delivery gate"
[ -f "$SKILL_DIR/core/gate-src/flaky-kit-delivery-gate.sh" ] && ok || bad "the delivery gate needs a restore source like its sibling"
...
"$KITSRC/install.sh"  --project "$P3" >/dev/null 2>&1
S3="$P3/.cursor/hooks.json"
...
SH
chmod +x "$SHIM/mktemp"
OUT="$(MKTEMP_DIR="$TD" PATH="$SHIM:$PATH" "$KITSRC/install.sh"  --project "$P3" 2>&1)"; RC3=$?
...
[ "$(cat "$P3/.cursor/skills/hektor-flaky-triage/core/.harness")" = "claude stop" ] \
...
# the end". That claim needs work scheduled AFTER the Stop merge, and with  there is
# none; it is asserted on its own fixture below.
[ -x "$P3/.cursor/hooks/flaky-kit-self-protection-gate.sh" ] \
  && ok || bad "the failing path must still install the self-protection gate — the merge that failed is a different control"
[ -x "$P3/.cursor/hooks/flaky-kit-delivery-gate.sh" ] \
...
[ -f "$P3/.cursor/skills/hektor-flaky-triage/core/gate-src/flaky-kit-delivery-gate.sh" ] \
...
# AGENTS.md block are the only work scheduled AFTER the Claude Stop merge, so `` is the
...
mkdir -p "$P4/.claude"
printf '{"hooks":{"stop":"not-an-array"}}\n' > "$P4/.cursor/hooks.json"
OUT4="$("$KITSRC/install.sh"  --project "$P4" 2>&1)"; RC4=$?
[ "$RC4" = 74 ] && ok || bad "CONTROL: the  fixture must also hit the failing Stop merge — got rc $RC4"
...
OUTH="$("$KITSRC/install.sh"  --project "$P" 2>&1)"; RCH=$?
case "$OUTH" in
...
OUT5="$(MKTEMP_DIR="$TD2" PATH="$SHIM:$PATH" "$KITSRC/install.sh"  --project "$P5" 2>&1)"; RC5=$?
...
[ "$(cat "$P5/.cursor/skills/hektor-flaky-triage/core/.harness")" = "cursor" ] \
...
[ -f "$P5/.cursor/skills/hektor-flaky-triage/core/gate-src/flaky-kit-self-protection-gate.sh" ] \
...
# the Cursor merge, so a pre-broken `.cursor/hooks.json` under `` is what tells "finished,
...
OUT6="$("$KITSRC/install.sh"  --project "$P6" 2>&1)"; RC6=$?
[ "$RC6" = 74 ] && ok || bad "CONTROL: the  fixture must also hit the failing Cursor merge — got rc $RC6"
[ "$(jq -r '[.hooks.stop[]?|(.hooks//[])[]?|.command|select(test("flaky-kit-delivery-gate"))]|length' "$P6/.cursor/hooks.json")" = 1 ] \
...
OUT7="$("$KITSRC/install.sh"  --project "$P7" 2>&1)"; RC7=$?
case "$OUT7" in
...
# wired (.cursor/hooks.json: preToolUse Write|Edit + Bash, Stop)" and "install: done", named
...
P8="$TMP/proj-copyfail"; mkdir -p "$P8/.cursor/skills/hektor-flaky-triage"; git -C "$P8" init -q
chmod 500 "$P8/.cursor/skills/hektor-flaky-triage"
OUT8="$("$KITSRC/install.sh"  --project "$P8" 2>&1)"; RC8=$?
chmod 700 "$P8/.cursor/skills/hektor-flaky-triage"
...
[ ! -f "$P8/.cursor/skills/hektor-flaky-triage/core/gate.sh" ] \
...
S8="$P8/.cursor/skills/hektor-flaky-triage"
...
OUT8B="$("$KITSRC/install.sh"  --project "$P8" 2>&1)"; RC8B=$?
...
P8C="$TMP/proj-copyfail-empty"; mkdir -p "$P8C/.cursor/skills/hektor-flaky-triage/core"; git -C "$P8C" init -q
chmod 500 "$P8C/.cursor/skills/hektor-flaky-triage/core"
OUT8C="$("$KITSRC/install.sh"  --project "$P8C" 2>&1)"; RC8C=$?
chmod 700 "$P8C/.cursor/skills/hektor-flaky-triage/core"
...
rm -f "$GSRC/gates/flaky-kit-delivery-gate.sh"
P9="$TMP/proj-gatefail"; mkdir -p "$P9"; git -C "$P9" init -q
OUT9="$("$GSRC/install.sh"  --project "$P9" 2>&1)"; RC9=$?
...
/bin/chmod -x "$GSRC2/gates/$SPG"
P10="$TMP/proj-gatenoexec"; mkdir -p "$P10"; git -C "$P10" init -q
OUT10="$(CHMOD_SKIP="$P10/.cursor/hooks/$SPG" PATH="$SHIM:$PATH" "$GSRC2/install.sh"  --project "$P10" 2>&1)"; RC10=$?
[ -f "$P10/.cursor/hooks/$SPG" ] && [ ! -x "$P10/.cursor/hooks/$SPG" ] \
...
case "$OUT10" in *"NOT EXECUTABLE: $P10/.cursor/hooks/$SPG"*) ok ;; *) bad "the failure must distinguish a non-executable gate from a missing one — got: $OUT10" ;; esac
...
# `cursor.sh`, and the gate's own header is
# `if [ -f "$_COMPAT" ]; then . "$_COMPAT"; else exit 0; fi` — so a missing compat lib does not
# degrade the gate, it disables it. `` is the default, so this is the ordinary path.
...
OUTC1="$("$KITSRC/install.sh"  --project "$PC1" 2>&1)"; RCC1=$?
...
case "$OUTC1" in *"COPY FAILED: $PC1/.cursor/hooks/lib/cursor.sh"*) ok ;; *) bad "the failure must name cursor.sh, whose absence disables the gate outright — got: $OUTC1" ;; esac
...
CURPAY='{"hook_event_name":"beforeShellExecution","command":"rm -rf .cursor/skills/hektor-flaky-triage"}'
GOUT1="$( cd "$PC1" && printf '%s' "$CURPAY" | "$CURGATE" 2>&1 )"
case "$GOUT1" in *'"permission": "deny"'*) bad "CONTROL: with cursor.sh absent the gate is expected to allow silently — if it denies, this fixture no longer models the hole" ;; *) ok ;; esac
...
cp "$KITSRC/gates/lib/cursor.sh" "$PC1/.cursor/hooks/lib/cursor.sh"
GOUT2="$( cd "$PC1" && printf '%s' "$CURPAY" | "$CURGATE" 2>&1 )"
case "$GOUT2" in *'"permission": "deny"'*) ok ;; *) bad "CONTROL: with cursor.sh present the gate must deny — got: $GOUT2" ;; esac
...
PC2="$TMP/proj-gatesrcfail"; mkdir -p "$PC2/.cursor/skills/hektor-flaky-triage/core/gate-src"; git -C "$PC2" init -q
chmod 500 "$PC2/.cursor/skills/hektor-flaky-triage/core/gate-src"
OUTC2="$("$KITSRC/install.sh"  --project "$PC2" 2>&1)"; RCC2=$?
chmod 700 "$PC2/.cursor/skills/hektor-flaky-triage/core/gate-src"
[ -z "$(ls -A "$PC2/.cursor/skills/hektor-flaky-triage/core/gate-src" 2>/dev/null)" ] \
...
[ -f "$PC2/.cursor/skills/hektor-flaky-triage/core/gate.sh" ] \
...
case "$OUTC2" in *"$PC2/.cursor/skills/hektor-flaky-triage/core/gate-src"*) ok ;; *) bad "the failure must name core/gate-src — got: $OUTC2" ;; esac
...
# fixture (.cursor/hooks/lib/audit.sh) and one fails (gate-src/lib/audit.sh), so the count is
...
# under gate-src/lib was failing too and raising the alarm on their behalf. Pre-seeding that
...
mkdir -p "$PC2B/.cursor/skills/hektor-flaky-triage/core/gate-src/lib"; git -C "$PC2B" init -q
GS2B="$PC2B/.cursor/skills/hektor-flaky-triage/core/gate-src"
printf 'pre-seeded so vendor() skips\n' > "$GS2B/lib/audit.sh"
chmod 500 "$GS2B"
OUTC2B="$("$KITSRC/install.sh"  --project "$PC2B" 2>&1)"; RCC2B=$?
chmod 700 "$GS2B"
...
# `.cursor/hooks.json` written so the gate is REGISTERED, and gate-src/ with no gate in it.
...
mkdir -p "$PC2C/.cursor/skills/hektor-flaky-triage/core/gate-src/lib"; git -C "$PC2C" init -q
GS2C="$PC2C/.cursor/skills/hektor-flaky-triage/core/gate-src"
printf 'pre-seeded so vendor() skips\n' > "$GS2C/lib/audit.sh"
chmod 500 "$GS2C"
OUTC2C="$("$KITSRC/install.sh"  --project "$PC2C" 2>&1)"; RCC2C=$?
chmod 700 "$GS2C"
...
[ -f "$PC2C/.cursor/skills/hektor-flaky-triage/core/gate.sh" ] && [ -x "$PC2C/.cursor/hooks/flaky-kit-self-protection-gate.sh" ] \
...
"$KITSRC/install.sh"  --project "$PC3" >/dev/null 2>&1
SC3="$PC3/.cursor/skills/hektor-flaky-triage"
printf '\n# STALE MARKER\n' >> "$SC3/core/gate.sh"
chmod -R a-w "$SC3/core"
OUTC3="$("$KITSRC/install.sh"  --project "$PC3" 2>&1)"; RCC3=$?
...
OUTC3B="$("$KITSRC/install.sh"  --project "$PC3" 2>&1)"; RCC3B=$?
...
"$KITSRC/install.sh"  --project "$PC4" >/dev/null 2>&1
SC4="$PC4/.cursor/skills/hektor-flaky-triage"
chmod a-w "$SC4/core/README.md"
OUTC4="$("$KITSRC/install.sh"  --project "$PC4" 2>&1)"; RCC4=$?
...
rm -rf "$P/docs/hektor"
( cd "$P" && printf '' | "$P/.cursor/hooks/flaky-kit-delivery-gate.sh" >/dev/null 2>&1 )
...
# — gates/SKILL.md, gates/lib/audit.sh, etc. — still resolve when it runs from the copy.
...
OUTLH="$("$SRCH/install.sh"  --project "$PH" 2>&1)"; RCLH=$?
SKILL_H="$PH/.cursor/skills/hektor-flaky-triage"
...
OUTLU="$("$SRCU/install.sh"  --project "$PU" 2>&1)"; RCLU=$?
SKILL_U="$PU/.cursor/skills/hektor-flaky-triage"
...
SYMOUT="$("$SYM/bin/hektor-triage-kit" install  --project "$SYM/proj" 2>&1)"; SYMRC=$?
[ "$SYMRC" -eq 0 ] \
  && ok || bad "install through a symlinked CLI must succeed (rc=$SYMRC): $(printf '%s' "$SYMOUT" | tail -1)"
[ -f "$SYM/proj/.cursor/skills/hektor-flaky-triage/core/apply.sh" ] \
  && ok || bad "install through a symlinked CLI must lay down the engine under .claude/"
[ -f "$SYM/proj/.cursor/skills/hektor-flaky-triage/SKILL.md" ] \
...
for want in core/ skill/ gates/ install.sh hektor-triage-kit README.md kernel.md enforcement-codeowners.md; do
  case "$want" in
...
( cd "$VER_P1" && git init -q . && bash "$VER_SRC/kit/install.sh"  >/dev/null 2>&1 )
V1="$VER_P1/.cursor/skills/hektor-flaky-triage/core/.version"
...
[ -f "$VER_P1/.cursor/skills/hektor-flaky-triage/core/.gitignore" ] \
...
sleep 1
( cd "$VER_P2" && git init -q . && bash "$VER_SRC/kit/install.sh"  >/dev/null 2>&1 )
V2="$VER_P2/.cursor/skills/hektor-flaky-triage/core/.version"
...
if [ -n "$TGZ" ]; then
  ...
  ( cd "$TP" && git init -q . && bash "$UNP/package/install.sh"  >/dev/null 2>&1 )
  [ -f "$TP/.cursor/skills/hektor-flaky-triage/core/.gitignore" ] \
    && ok || bad "an install from the TARBALL must carry core/.gitignore downstream — cp -R can only carry what shipped"
  [ -f "$TP/.cursor/skills/hektor-flaky-triage/core/.version" ] \
  ...
  ( cd "$NPP" && git init -q . && "$NPX/bin/hektor-triage-kit" install  >/dev/null 2>&1 )
  NPS="$NPP/.cursor/skills/hektor-flaky-triage"
  [ -f "$NPS/core/apply.sh" ] \
  ...
  ( cd "$NPP" && git check-ignore -q ".cursor/skills/hektor-flaky-triage/core/.lock-state" ) \
  ...
  ( cd "$NPP" && "$NPX/bin/hektor-triage-kit" install  >/dev/null 2>&1 )
...
case "$DOUT" in *gates/*) ok ;; *) bad "build must name the file the scan refused, which only the scan's own output knows" ;; esac
...
BINST="$(mktemp -d)"
( cd "$BINST" && git init -q . && bash "$BLD/kit/install.sh"  >/dev/null 2>&1 )
cp "$BLD/kit/hektor-triage-kit" "$BINST/.cursor/skills/hektor-flaky-triage/hektor-triage-kit"
( cd "$BINST/.cursor/skills/hektor-flaky-triage" && bash ./hektor-triage-kit build >/dev/null 2>&1 ); IRC=$?
