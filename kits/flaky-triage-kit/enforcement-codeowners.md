...
> **Status in THIS repo: INERT (forward-looking).** `web-test/.gitignore` ignores `.cursor/`, and
...
> 1. `/.cursor/hooks/flaky-kit-self-protection-gate.sh` — preToolUse gate, installed **outside** the
...
/.cursor/skills/hektor-flaky-triage/         @qa-automation
/.cursor/hooks/flaky-kit-self-protection-gate.sh @qa-automation
/.cursor/hooks/flaky-kit-delivery-gate.sh    @qa-automation
/.cursor/hooks/lib/audit.sh                  @qa-automation
/.cursor/hooks/flaky-kit-self-protection-gate.sh @qa-automation
...
`<project>/.cursor/hooks/` precisely so a rename of the kit tree cannot take it along.)
