...
#   hooks/lib/audit.sh                       (the pack — installed to .cursor/hooks/lib/)
#   kits/flaky-triage-kit/gates/lib/audit.sh (the kit — vendored to .cursor/hooks/lib/)
# Both append to the SAME docs/hektor/.hook-audit.log, so bypass use is recorded in one place even
# though two installers put the file there. Drift check is a plain `diff` of the two paths — keep it
