...
| **I11** | **A session may not end while any cluster is `selected`/`applied`** — run `core/ledger.sh validate --final <ledger>` before the convergence summary; non-zero exit means the run is NOT done (collect verdicts; never background the green-proof and quit). **Checked either way:** on Claude Code the Stop hook (`<project>/.claude/hooks/flaky-kit-delivery-gate.sh`, source `adapters/claude/`) derives the ledger path from the transcript and runs this same `validate --final` at every stop — no `stop_hook_active` escape, no environment bypass. Cursor has no Stop event, so there this row is still the agent's own discipline. | premature completion / unverified "fixed" claims |
