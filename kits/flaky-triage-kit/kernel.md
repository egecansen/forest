...
| **I11** | **A session may not end while any cluster is `selected`/`applied`** — run `core/ledger.sh validate <ledger> --final` before the convergence summary (that order, not `validate --final <ledger>` — the file is the subcommand's first positional and the flag comes after it; the reversed spelling exits **65** “not a ledger” instead of I11's own **67**, i.e. it reads as a broken file rather than as an open cluster, and the gate below uses the correct order); non-zero exit means the run is NOT done (collect verdicts; never background the green-proof and quit). **Checked either way:** on Claude Code the Stop hook (`<project>/.claude/hooks/flaky-kit-delivery-gate.sh`, source `adapters/claude/`) derives the ledger path from the transcript and runs this same `validate --final` at every stop — no `stop_hook_active` escape, no environment bypass. Cursor has no Stop event, so there this row is still the agent's own discipline. | premature completion / unverified "fixed" claims |
