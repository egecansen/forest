...
/**
 * The app has exactly three views: the start screen, a live run console
 * (backed by a WS stream), and a read-only console for a past, persisted
 * run opened from the history list. Only 'console' ever has a stoppable/
 * interruptible run behind it.
 */
type View =
  | { kind: 'start' }
...
export function App() {
  ...
  // start screen. Only runs once, and only adopts a run when we're still on the
  // start screen (never clobbers an in-progress interaction). (root fix)
  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        ...
        setActiveConfig(cfg);
        setView((v) => (v.kind === 'start' ? { kind: 'console', config: cfg } : v));
      } catch {
  ...
  // there is one (re-opening its console reconnects the WS), else the start screen.
  const leaveHistory = useCallback(() => {
    setView(activeConfig ? { kind: 'console', config: activeConfig } : { kind: 'start' });
  }, [activeConfig]);
  ...
  const viewKey =
    view.kind === 'console' ? view.config.runId : view.kind === 'history' ? `history-${view.runId}` : 'start';
  ...
  return (
    <div className="app-shell">
      ...
      <div className="view-switch" key={viewKey}>
        {view.kind === 'start' && <StartScreen onStart={startRun} />}
        {view.kind === 'console' && (
          <RunConsole
            config={view.config}
            onStop={stopRun}
            ...
            <div className="start-screen">
              <div className="start-card">
                ...
                <button type="button" className="btn btn-ghost" onClick={leaveHistory}>
                  {activeConfig ? 'back to session' : 'back to start'}
                </button>
              </div>
            </div>
          )
