...
test('/api/worktree/priority broadcasts a fresh snapshot so the deck recolors', async () => {
  const repo = await tmp('forest-prio-route-');
  try {
    const calls = [];
    const ctx = prioCtx(repo, calls);
    let broadcasted = false;
    ctx.broadcast = () => { broadcasted = true; };
    await callPrio({ path: WT_PATH, priority: 'blue' }, ctx);
    assert.ok(calls.includes('fresh'), 'the recolor must come from a fresh snapshot');
    assert.ok(broadcasted, 'other clients hear about the new color');
  } finally { await rm(repo, { recursive: true, force: true }); }
});
