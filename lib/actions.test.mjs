...
// The convention branches: a kit with no installer still writes wiring if it
// ships a hooks dir to copy or a settings fragment to merge.
for (const [label, files] of [
  ...
  test(`launchDecision: repairable is true for a kit with no install.sh but ${label}`, async () => {
    ...
    assert.equal(d.repairable, true,
      'provisionKit falls back to the conventions when a kit ships no installer, and both of these write hook wiring');
  });
}
