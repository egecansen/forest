...
import { worktreeTitle, runSelections, launchDecision, orphanedUnits, createActionHandler, recordWithout } from './actions.mjs';
...
test('/api/launch: agent cursor with a selection that ships no install.sh runs nothing and says so in the journal', async () => {
  const { wt, ctx, cleanup } = await cursorLaunchFixture();
  try {
    const adapterCalls = [];
    const res = fakeRes();
    ...
    assert.ok(ctx.journal.entries.some((e) => /no pack in the selection ships an install\.sh/.test(e.cmd)), JSON.stringify(ctx.journal.entries));
  } finally { await cleanup(); }
});
