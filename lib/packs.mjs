...
export async function copyTree(src, dest, { owner = 'unknown', conflicts = [], written = new Map(), overwrite = false, updatedPaths = [] } = {}) {
  ...
  for (const e of entries) {
    ...
    if (existing && sha(existing) !== sha(incoming)) {
      ...
      // A destination the process cannot write — a hardened (root-owned)
      // kit file, or one made read-only by hand — is a collision to report,
      // never an error to throw: throwing here failed the WHOLE provision
      // ("Launch failed: provision failed: EACCES", web-test, 2026-09-09)
      // over one file the kit's own installer refuses to touch as well.
      try {
        await copyFile(from, to);
      } catch (e) {
        if (e.code !== 'EACCES' && e.code !== 'EPERM') throw e;
        conflicts.push({ path: to, incoming: owner, existing: other || 'preexisting', reason: 'unwritable' });
        continue;
      }
      updated += 1;
    ...
    // Hooks must stay executable. A mode change refused on a file this
    // process does not own (an identical, hardened copy) is not a failure:
    // the content already matches and the owner keeps the mode.
    try {
