import { readdir, readFile, writeFile, mkdir, copyFile, chmod, stat, appendFile, unlink, symlink } from 'node:fs/promises';
...
export async function copyTree(src, dest, { owner = 'unknown', conflicts = [], written = new Map(), overwrite = false } = {}) {
  let entries;
  try { entries = await readdir(src, { withFileTypes: true }); } catch { return { copied: 0, updated: 0 }; }
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      const r = await copyTree(from, to, { owner, conflicts, written, overwrite });
      copied += r.copied;
      updated += r.updated;
      continue;
    }
...
export async function provisionKit({ kitDir, kitId, worktreePath, conflicts, written, notes = [], refresh = false }) {
  const man = await readKitManifest(kitDir);
  const claudeDir = join(worktreePath, '.claude');
  const opts = { owner: kitId, conflicts, written, overwrite: refresh };
...
export async function provisionPack({ packsDir, pack, skills = [], kits = [], hooks = false, worktreePath, refresh = false }) {
  ...
  const opts = (owner) => ({ owner, conflicts: out.conflicts, written, overwrite: refresh });
  ...
  for (const id of kits) {
    ...
    const r = await provisionKit({ kitDir: join(packDir, 'kits', id), kitId: id, worktreePath, conflicts: out.conflicts, written, notes: out.notes, refresh });
    out.kits.push(id);
