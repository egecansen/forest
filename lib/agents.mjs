...
import { runInTerminal, launchClaudeSession } from './terminal.mjs';
...
export function launchInteractive({ worktreePath, app = 'Terminal', title }) {
  // One Claude session per worktree: reuse (front) if alive, else launch.
  return launchClaudeSession({ worktreePath, app, title });
}
