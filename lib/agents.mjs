...
import { runInTerminal, terminalSession } from './terminal.mjs';
...
export function launchInteractive({ worktreePath, app = 'Terminal' }) {
  // Reuse the existing claude tab for this worktree if one is running;
  // otherwise open a new tagged session.
  return terminalSession({ marker: `Forest claude · ${worktreePath}`, body: 'claude', cwd: worktreePath, app, requireBusy: true });
}
