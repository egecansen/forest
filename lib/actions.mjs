  ...
  return async function handleAction(req, res, ctx, readBody) {
    ...
    try {
      ...
      if (url === '/api/launch') {
        ...
        const sel = selections.filter((s) => s && s.pack && ((s.skills?.length) || (s.kits?.length)));
        let provisioned = null;
        if (sel.length) {
          provisioned = { skills: [], kits: [] };
          try {
            for (const s of sel) {
              const r = await provisionPack({ packsDir: ctx.config.packsDir, pack: s.pack, skills: s.skills || [], kits: s.kits || [], worktreePath: path });
              provisioned.skills.push(...r.skills);
              provisioned.kits.push(...r.kits);
            }
            const n = provisioned.skills.length, k = provisioned.kits.length;
            ctx.journal.add({ cmd: `provision: ${n} skill(s)${k ? `, ${k} kit(s)` : ''} → .claude/`, cwd: path, mode });
          } catch (e) {
