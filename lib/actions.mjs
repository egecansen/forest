...
// genuinely undecidable: a kit that ships `install.sh` owns its own wiring,
// and forest cannot know which files that installer writes. Two other cases
// looked like over-approximations at first read — a kit whose `hooks/`
// directory exists but is empty, and a kit whose `settings.hooks.json` is
// present but not valid JSON — but both are refutable by a content check
// (`readdir`, `JSON.parse`), and `writesHookWiring` performs both. Everything
// else is refutable, and refuted where it is false.
//
