...
import { provisionPack, writeProvisionRecord, readProvisionRecord, safeId, writesHookWiring, ensureExcluded, listPacks, autoSelections, packFingerprint } from './packs.mjs';
...
export async function runSelections({ ctx, path, selections, mode, refresh = false }) {
  ...
  for (const c of out.conflicts) {
    ctx.journal.add({
      cmd: c.reason === 'unwritable'
        ? `refresh skipped: ${c.path} is not writable (root-owned or read-only, a hardened install) — kept existing; unlock it to take the pack's version`
        : `collision: ${c.path} (${c.incoming} ≠ ${c.existing}) — kept existing`,
      cwd: path, mode,
    });
  }
...
export function createActionHandler({
  launch = launchInteractive, runTerminal = runInTerminal, fingerprint = packFingerprint, resolveScope = resolveSessionScope,
  ...
  async function ensureProvisioned({ ctx, path, repoName, selections = null, mode, reason, force = false }) {
    ...
    // even with autoProvision off, per the comment above `explicit`) — so
    // `auto` is always true here.
    if (!explicit && ctx.config.autoProvision === false) return { skipped: 'autoProvision off', auto: true };
    let sel;
    if (explicit) {
    ...
    // `force` means "provision now": the "Launch anyway" retry after the
    // orphan dialog re-sends the (now smaller) subset with force: true, and
    // that subset is by construction already fully present in the record —
    // unchangedSince would say skip, leaving the record still naming the
    // deselected unit and re-blocking the very next launch on it. Skip is
    // only for the UNFORCED path, where "nothing to do" is actually true.
    if (!force && unchangedSince(previous, sel, fingerprints, cursorPacks)) {
  ...
  return async function handleAction(req, res, ctx, readBody) {
    ...
    try {
      if (url === '/api/worktree/create') {
        ...
        if (mode === 'guided') {
          // createWorktreeAt appends the provision-notify curl clause only
          // when autoProvision is not false (see its `after` computation) —
          // this must not promise a notify that was never sent.
          const provisioning = ctx.config.autoProvision === false
            ? 'off (autoProvision is false)'
            : 'on first launch or when the Terminal command finishes';
          return sendJson(res, { ...result, provisioning });
        }
      ...
      if (url === '/api/worktree/apply-diff') {
        ...
        if (!(await managedPath(ctx, sourcePath)).ok || !(await managedPath(ctx, targetPath)).ok) {
          return sendJson(res, { error: PATH_ERROR }, 400);
        }
      ...
      if (url === '/api/worktree/finish') {
        ...
        // Both paths are acted on: the worktree at `path` is what gets
        // renamed/detached/removed, and `repoPath` is the main checkout that
        // gets switched and merged into — the same two fields /api/worktree/remove
        // already validates.
        if (!(await managedPath(ctx, repoPath)).ok || !(await managedPath(ctx, path)).ok) return sendJson(res, { error: PATH_ERROR }, 400);
        if (mode === 'guided') {
      ...
      if (url === '/api/worktree/eject') {
        const { repoPath } = body;
        // The only path in this route's body: eject reads the worktree path
        // it recreates from the landing ledger, not from the request.
        if (!(await managedPath(ctx, repoPath)).ok) return sendJson(res, { error: PATH_ERROR }, 400);
        if (mode === 'guided') {
      ...
      if (url === '/api/launch') {
        ...
        return r && r.ok
          ? sendJson(res, {
            ok: true, action: r.action, agent: r.agent || agent, provisioned, provision,
