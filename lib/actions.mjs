...
//
// `true` remains an over-approximation in exactly one place, and only one: a
// kit that ships `install.sh` owns its own wiring, and forest cannot know
// which files that installer writes. Everything else is now refutable, and
// refuted where it is false.
//
