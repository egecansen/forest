...
// Observed 2026-09-09 on web-test: its .claude/ holds a hardened (root-owned)
// flaky-kit install, and the pack's hooks/lib/audit.sh differs from the kit's
// copy at the same path. The refresh every launch now does tried to overwrite
// it, copyFile threw EACCES, and the WHOLE provision failed — "Launch failed:
// provision failed: EACCES" — where the pre-refresh code had recorded a
// collision and kept going. An unwritable destination is a collision to
// report, never an error to throw.
test('copyTree refresh: an unwritable (hardened) destination is reported as a collision, kept, and never throws', async () => {
  const src = await tmp('forest-src-'), dst = await tmp('forest-dst-');
  try {
    await writeFile(join(src, 'audit.sh'), 'pack version\n');
    await writeFile(join(src, 'other.sh'), 'new file\n');
    await writeFile(join(dst, 'audit.sh'), 'hardened version\n');
    await chmod(join(dst, 'audit.sh'), 0o444);   // read-only: the owner cannot open it for writing either
    const conflicts = [];
    const r = await copyTree(src, dst, { owner: 'hektor-gates', overwrite: true, conflicts });
    assert.equal(r.updated, 0);
    assert.equal(r.copied, 1, 'the writable sibling still lands');
    assert.deepEqual(conflicts, [{ path: join(dst, 'audit.sh'), incoming: 'hektor-gates', existing: 'preexisting', reason: 'unwritable' }]);
    assert.equal(await readFile(join(dst, 'audit.sh'), 'utf8'), 'hardened version\n');
    assert.equal(await readFile(join(dst, 'other.sh'), 'utf8'), 'new file\n');
  } finally {
    await chmod(join(dst, 'audit.sh'), 0o644).catch(() => {});
    await rm(src, { recursive: true, force: true });
    await rm(dst, { recursive: true, force: true });
  }
});

test('copyTree: an identical file whose mode cannot be changed is left alone without an error', async () => {
  const src = await tmp('forest-src-'), dst = await tmp('forest-dst-');
  try {
    await writeFile(join(src, 'same.sh'), 'same\n');
    await chmod(join(src, 'same.sh'), 0o755);
    await writeFile(join(dst, 'same.sh'), 'same\n');
    await chmod(join(dst, 'same.sh'), 0o444);
    const conflicts = [];
    const r = await copyTree(src, dst, { owner: 'hektor-gates', overwrite: true, conflicts });
    assert.equal(r.copied + r.updated, 0);
    assert.deepEqual(conflicts, []);
  } finally {
    await chmod(join(dst, 'same.sh'), 0o644).catch(() => {});
    await rm(src, { recursive: true, force: true });
    await rm(dst, { recursive: true, force: true });
  }
});
