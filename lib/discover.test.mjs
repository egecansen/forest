...
test('buildSnapshot includes a listed repo that the scan cannot reach', async () => {
  const { repo } = await makeRepoWithWorktree({ branch: 'tech/LIST-1' });
  ...
  try {
    ...
    // git canonicalizes worktree paths (see the findRecord comment above for
    // why /var vs /private/var symlinks make raw path comparison flaky on
    // macOS) — match by branch instead, same as findRecord does.
    assert.ok(rec.worktrees.some((w) => w.branch === 'tech/LIST-1'));
  } finally {
    await rm(repo, { recursive: true, force: true });
