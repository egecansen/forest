...
test('executeFinish: a dirty main checkout is fine when no branch switch is needed', async () => {
  const { repo, wt } = await makeRepoWithWorktree();
  try {
    await executeFinish({ repoPath: repo, path: wt, remove: false }); // main now holds the branch
...
// Set up as a RE-landing (first finish leaves main holding the branch, so the
// second one switches nothing). Before the dirty-main guard this test dirtied
...
test('executeFinish: two concurrent finishes on one repo — op lock serializes them, both land cleanly', async () => {
  ...
  try {
    ...
    const headBranch = (await git(repo, 'rev-parse', '--abbrev-ref', 'HEAD')).trim();
    assert.ok(['t/a', 't/b'].includes(headBranch), `HEAD should be one of t/a or t/b, got ${headBranch}`);
